...
- S1-01's internal admission probe pauses command 0 first after sequence reservation and before append while the owner lock remains held, then after the admission critical section and before drain submission. The same test proves `0, 1` callback order in correct code and fails against a temporary mutant that moves command 0's append after the second hook; no sleep is permitted.
...
- S1-01 runs the 18 allowlisted XCTest cases with `--xunit-output`; a parser requires exactly 18 executed entries and zero skipped, disabled, failures, or errors at aggregate and case level. A temporary `XCTSkip` canary must fail this execution gate. Method 18 binds `wallet-01\0mainnet\0thorchain-1` to `e2df225b7a00d471b1b09ec2d3344df89a11e9cfe116c05f5290683480623015`, and separator/order source mutants must fail.
