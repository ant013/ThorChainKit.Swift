...
**Status:** revision 10 after revision-9 adversarial REVISE; implementation blocked pending fresh review and explicit approval.
...
- `Scripts/verify-s1-01-factory.swift` owns a positive normalized syntax/callee audit for exactly `Core/KitFactory.swift`, `Core/KitDependencies.swift`, `Core/Kit.swift`, and the exact `Network.persistenceKey` declaration/body in `Models/Network.swift`. It canonicalizes the pinned compiler's parsed declarations, imports, identifier/member references, and call shapes and requires exact equality with `Fixtures/S1-01-factory-syntax.txt`; the fixture permits only validated value construction, `NoOpLifecycle`, `Kit`, the dedicated serial facade `DispatchQueue(label:)`, the owner `NSLock`, the required current-value subjects, one retained `DispatchSpecificKey<UInt8>()`, one `facadeDispatcher.setSpecific(key:value:)`, and `DispatchQueue.getSpecific(key:)` reads of that same key. An extra helper, import, call, member reference, alias, wrapper, or global-queue submission fails before any blacklist check. Temporary-copy canaries inject `URLSession.shared`, `URLRequest`, `Data(contentsOf:)`, `FileManager.default`, `FileHandle(forUpdatingAtPath:)`, `UserDefaults.standard`, `sqlite3_open`, `Task {}`, `OperationQueue`, `DispatchQueue.global().async`, `Timer.scheduledTimer`, `DispatchSource.makeTimerSource`, an alias, a wrapper inside an allowed file, a helper outside the exact path, and `Data(contentsOf:)` inside `Network.persistenceKey`; every canary must fail the same named audit.
- `Scripts/verify-s1-01-values.swift` owns a separate positive normalized construction audit for the complete public-value closure used by the external consumer: the public initializers and exact transitive validation bodies in `Models/Network.swift`, `Network/EndpointFamilyDescriptor.swift`, `Network/EndpointPolicy.swift`, `Models/EndpointConfiguration.swift`, `Models/Denom.swift`, `Models/Address.swift`, `Address/Bech32Codec.swift`, and `Address/BitConversion.swift`. Parsed declarations, imports, identifier/member references, and call shapes must equal `Fixtures/S1-01-value-syntax.txt`; a helper or callee outside that enumerated closure fails. `Scripts/verify-s1-01.sh` runs the positive baseline and four guarded one-change temporary-copy mutants: `Data(contentsOf:)` and `Task {}` inserted separately into `Address` construction, and the same two capabilities inserted separately into endpoint construction. All four must fail this same positive audit before the public-only consumer build runs.
- an internal initializer accepts `KitDependencies` for deterministic lifecycle tests. The only additional test seam is an internal `LifecycleAdmissionProbe` with two callbacks: `afterSequenceReservedBeforeAppend`, invoked while the owner lock remains held, and `afterAdmissionBeforeDrainSubmission`, invoked after the admission critical section when the correct implementation has already appended. The first hook may only emit one precreated, bounded, nonblocking test signal and return; it must not call any `Kit` API, wait for another thread, block, or reenter lifecycle. The second hook may perform the barrier wait used by the FIFO regression because it runs after the owner lock is released. The public factory cannot supply the probe and no dependency type is public.
...
- A lifecycle call made synchronously by a lifecycle collaborator already executing on the facade dispatcher must not wait behind the active drain. `Kit` retains one private `DispatchSpecificKey<UInt8>`, installs value `1` on the dedicated dispatcher once with `facadeDispatcher.setSpecific(key:value:)`, and recognizes dispatcher context only when `DispatchQueue.getSpecific(key:) == 1` for that same key. An effective reentrant `start`, `stop`, or running `refresh` still linearizes and appends under the owner lock, but returns after enqueue so the active drain can continue to the queued command. A process-wide or owner-state flag such as `isDraining` is forbidden because it cannot distinguish an unrelated caller while a drain is active. This positively allowlisted dispatcher-key check is the sole exception to synchronous effective-call completion.
...
- S1-01 tests use barriers/expectations and explicit call-entry control, never sleeps, to exercise both legal overlap orders, owner-lock FIFO append/callback order, exact callback counts, and final desired state. In method 16, command 0's first hook emits its nonblocking under-lock signal and returns. Its second hook blocks after command 0 has been appended and the owner lock released; the test then starts command 1 and releases command 0, proving callback order `0, 1`. `Scripts/test-s1-01-mutants.sh` is a nonrecursive outer harness: it runs method 16 once as the baseline, makes a tracked-source temporary copy, applies exactly one guarded mutation moving command 0's append after the second hook, and directly reruns only method 16 in the copy without invoking `verify-s1-01.sh`; that mutant must exit nonzero because command 1 overtakes command 0. This is the required falsifiable control of the former post-unlock/pre-append gap.
- A barrier-controlled table separately covers every effective dispatcher-context reentry branch by having the active lifecycle collaborator synchronously call `stop` while running, `start` while stopped, or `refresh` while running. Each reentrant method must return so the active collaborator can unwind and the same drain can continue. The ordinary-stop completion case starts from an unrelated thread while a preceding collaborator holds the active dispatcher drain: that stop must remain blocked until its own queued collaborator invocation returns, proving an `isDraining`-style false dispatcher classification fails. Publisher/subscriber and lifecycle collaborator callbacks never run under the owner lock. Reentrant no-op branches remain immediate. The admission probe's first nonblocking signal is the sole controlled under-lock test hook and may not reenter `Kit` or wait across threads.
...
| Required differences | Whitespace-only wallet rejection, address/network equality, collision-resistant internal `persistenceNamespace`, internal-only decoded address payload, one `desiredRunning` owner, owner-lock FIFO append, a two-hook bounded admission probe, exact `DispatchSpecificKey<UInt8>` dispatcher identity, collaborator-context reentry without a wait cycle, collaborators outside the owner lock, mandatory initial replay, idle/nil/zero initial state, no fake account/balance, and an auditable inert no-op factory including the transitive `Network.persistenceKey` getter. S1-01 adds no publication-turn machinery. |
...
| Tests before code | Invalid wallet/network inputs; exact positive factory syntax/callee fixture covering the transitive `Network.persistenceKey` getter with equivalent-capability, alias, wrapper, out-of-path-helper, and transitive-I/O canaries; both legal concurrent lifecycle orders and FIFO call counts; the two-hook admission test plus nonrecursive outer post-unlock/pre-append mutant harness; ordinary effective start/stop/refresh completion barriers, including an unrelated external stop during an active drain; barrier-controlled collaborator-context reentry for start/stop/refresh; immediate mandatory publisher replay with synchronous getters and lifecycle calls when no collaborator is waiting; idle/nil/zero/no-account snapshot. |
...
6. Lifecycle calls reserve sequence and append within the same owner-lock critical section. The bounded two-hook probe and nonrecursive outer temporary-copy harness that moves append after the post-admission hook make the former post-unlock/pre-append gap deterministically falsifiable. One retained `DispatchSpecificKey<UInt8>` positively identifies the facade dispatcher. Ordinary effective calls wait for completion even while another drain is active, while true collaborator-context dispatcher reentry appends and returns. S1-01 retains command draining and has no post-construction publication interface; S1-05 owns publication-turn admission, publication pre/post-draining, and deterministic `P0/C/P1` coverage.
...
18. Public value construction has its own positive normalized closure, independent of the Core factory closure. Address and endpoint I/O/task mutants must fail it. S1-04/S1-05 cross isolation only with internal `Sendable` decimal-string records and reconstruct BigUInt-backed public snapshots on the facade dispatcher.
...
- invoke non-executable `Scripts/verify-s1-01-values.swift` through `xcrun swift` to compare the exact public Network/endpoint/Denom/Address construction closure with `Fixtures/S1-01-value-syntax.txt`; require the Address I/O, Address task, endpoint I/O, and endpoint task temporary-copy mutants to fail that same positive gate;
- run executable `Scripts/test-s1-01-mutants.sh`; require one baseline pass, exactly one guarded transform per temporary copy, and direct nonrecursive mutant failures for method 16's moved append and method 18's separator/order changes;
...
- The exact positive public-value construction closure passes, and separate Address/endpoint I/O/task mutants fail before the external consumer build.
