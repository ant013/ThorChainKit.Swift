...
- No mnemonic/private key/API credential in Git, UserDefaults, fixtures, Maestro YAML, command line, process environment dumps, console, screenshots, or JUnit. The root `.env` containing `THORCHAIN_TEST_MNEMONIC` and `THORCHAIN_TEST_RECIPIENT_ADDRESS` is operator-owned, excluded by the tracked `.gitignore`, never printed/uploaded, and never included in CI/artifacts.
