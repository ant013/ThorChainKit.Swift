...
**Revision:** 3 — VOP-02 correction; discovery 1/2; closure 0/5
...
1. Use an explicitly selected Bash shell and make every run carry its own
   ...
   run_kit_probe baseline "$baseline_sha" 65
   run_kit_probe implementation "$IMPLEMENTATION_THORCHAIN_SHA" 0
   ```

   `PIPESTATUS[0]` is assigned immediately after each piped `xcodebuild`,
   before any later `rg`, hash, or bundle check can overwrite the compiler
   status. Each run's `DerivedData`, graph, checkout status, exit status, and
   log hash are retained under its printed `run_dir`.
...
4. Run the canonical Xcode 26.6 host command exactly from the already proven
   ...
   run_host_probe() {
     ...
     xcrun --find swiftc > "$run_dir/swiftc-path.txt"
     xcrun swiftc --version > "$run_dir/swiftc-version.txt"
     git -C "$THR160_SOURCE_PACKAGES/checkouts/ThorChainKit.Swift" rev-parse HEAD > "$run_dir/resolved-checkout-sha.txt"
     resolved_sha="$(cat "$run_dir/resolved-checkout-sha.txt")"
     test "$resolved_sha" = "$expected_thorchain_sha"
     test -s "$THR160_SOURCE_PACKAGES/workspace-state.json"
     cp "$THR160_SOURCE_PACKAGES/workspace-state.json" "$run_dir/resolved-host-package-graph.json"
     ...
     for pin_file in "${host_pin_files[@]}"; do
       pin_path="$THR160_HOST_ROOT/$pin_file"
       pin_key="${pin_file//\//_}"
       test -f "$pin_path"
     ...
     git -C "$THR160_SOURCE_PACKAGES/checkouts/ThorChainKit.Swift" status --short > "$run_dir/resolved-checkout-status.txt"
     printf '%s\n' "$THR160_HOST_ROOT" > "$run_dir/host-root.txt"
     printf '%s\n' "$THR160_SOURCE_PACKAGES" > "$run_dir/source-packages.txt"
   ...
   run_host_probe implementation "$IMPLEMENTATION_THORCHAIN_SHA" 0
   ```

   Each loop iteration checks each of the four pin files independently against
   its run-specific `expected_thorchain_sha`, rejects both known stale SHAs in
   each file, and checks the resolved SourcePackages checkout against the same
   value. `workspace-state.json` is copied and hashed as the resolved host
   package graph. `host_derived_data`, checkout status, toolchain files,
   captured exit, and log hash are all per-run artifacts. Do not claim success
   if compilation never reached the kit target.

5. Save the exact function invocation/command text, expected and resolved
   heads, all four individual pin-match outputs, toolchain and Xcode versions,
   resolved graph hash, fresh DerivedData and SourcePackages paths, captured
   exit statuses, log SHA-256 values, and selected positive `SwiftCompile`/
   diagnostic lines in
