...
| `sending Result<T, Error>` on private `CompletionGate.finish` | Conditional source delta | Models the one-way transfer consumed by `CheckedContinuation.resume(with:)`; implementation requires an exact A/B proof or the specified reduced canary. |
