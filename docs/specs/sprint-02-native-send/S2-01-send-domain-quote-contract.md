...
When `stop()` invalidates an unconsumed quote, `QuoteStore` retains an internal tombstone carrying the private authority envelope's opaque token, Kit namespace, lifecycle generation, and original monotonic deadline. Atomic `consume` changes an active record to a consumed tombstone before signer work. After lifecycle admission, lookup has one fixed order: verify the private Kit namespace (mismatch or an absent/cleaned authority envelope returns `quoteOwnershipMismatch`), then verify lifecycle generation (an old-generation envelope returns `quoteGenerationInvalidated`), then compare the authoritative monotonic deadline (`now >= deadline` returns `quoteExpired`), then inspect active/consumed state (a consumed pre-deadline envelope returns `quoteAlreadyConsumed`; only active pre-deadline state can be admitted). Tombstones are retained through the original deadline and lazily cleaned at that deadline; because the public quote cannot supply or reconstruct the private authority envelope, cleanup cannot make a token admissible. These precedence and cleanup rules are stable and are tested without exposing the token or envelope.
