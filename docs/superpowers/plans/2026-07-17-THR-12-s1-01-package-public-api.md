...
- One lock owns `desiredRunning`, sequence reservation, and FIFO append; an internal admission probe plus structural-mutation canary proves append atomicity. Publication turns drain lifecycle commands before and after synchronous delivery, so the deterministic competing-turn regression observes exact `P0 → C → P1` order.
...
- Test first: add only authoritative methods 1–12. Cover `thor/sthor/cthor` plus coin type 931; chain-ID 50-byte acceptance/51-byte rejection; denom grammar bounds; every endpoint rule (nonempty families, `https`, no credentials/query/fragment, normalized unique client IDs, lag/retry finiteness and sign, retry subset, page count, per-family attempts, and effective attempts); both directions of the AccountState invariant; stable SyncError cases; table-driven Address structure/canonical/classic-checksum/Bech32m/HRP/padding/payload failures; and the exact known-answer address-to-20-byte-payload vector under `@testable`.
...
- Test first: add only methods 13–18: whitespace-only wallet ID, address/network mismatch, category-complete no-work factory canaries, both legal overlap orders, an admission probe paused between sequence reservation and append plus a failing structural mutation, effective reentry for start/stop/refresh, deterministic `P0/C/P1`, mandatory replay, and namespace absence.
- Implementation: create one nonrecursive owner lock for snapshots/`desiredRunning`/sequence-and-append, one internal FIFO facade dispatcher, and atomic pre/post-publication lifecycle drains. A dispatcher-context effective call appends and returns without submitting a competing drain. Add only internal test dependencies/admission probe, the namespace, current-value publishers, and a factory backed by `NoOpLifecycle` plus the nil/idle/zero snapshot.
- Acceptance: start/stop transition once, repeated calls are filtered before the bridge, running refresh forwards once, stopped refresh no-ops, append cannot separate from reservation, exact `P0 → C → P1` holds, no collaborator runs under the lock, and the factory cannot create any audited capability or escape through an unlisted helper.
...
- Test first: after all 18 methods exist, prove that an extra product, forbidden import, extra public symbol, missing/extra test, wrong Xcode/Swift identity, strict-concurrency warning, iOS-16-only public API, public-consumer use of an internal symbol, each concrete forbidden factory capability, and composition through an unlisted helper each fail their own named canary.
...
- Implementation: pin `actions/checkout` to `34e114876b0b11c390a56381ad16ebd13914f8d5`, `actions/setup-java` to `c1e323688fd81a25caa38c78aa6df2d33d3e20d9`, and Maestro `2.6.1` `maestro.zip` to SHA-256 `3440825f514f537c6a96bcf5de995780c2a4a7f83a43208fdc95d4f1fecfad3`; require one UDID and root-absolute output paths. OCR recursively enumerates only regular in-root PNGs, rejects symlinks/path escapes, fails read/decode/OCR errors, and asserts enumerated equals processed.
