...
- The authoritative 18 `PublicApiTests` methods are staged with their owning implementation, then locked by an exact discovery allowlist and an xUnit gate proving all 18 executed once with zero skipped, disabled, failures, or errors.
...
- Whitespace-only wallet IDs and address/network mismatch fail with stable typed errors; the persistence namespace remains internal; the exact factory path creates no network, storage, task, timer, dispatch source, or unaudited helper capability.
- One lock owns `desiredRunning`, sequence reservation, and FIFO append; two bounded admission-probe hooks plus the same-test post-unlock/pre-append mutant prove append atomicity. S1-01 has no post-construction publication interface; S1-05 owns publication-turn admission and the deterministic `P0 → C → P1` regression.
...
- Command-shim canaries prove device argv, CLI/Java identity, immutable artifact provenance, and resolved artifact-path consistency. OCR recursively processes every regular in-root PNG, rejects symlinks/path escapes and any read/decode/OCR error, asserts enumerated equals processed, and fails safe-first/secret-second plus malformed-image canaries.
...
- Test first: add only methods 13–18: whitespace-only wallet ID, address/network mismatch, category-complete no-work factory canaries, both legal overlap orders, both admission-probe hooks plus a same-test failing post-unlock/pre-append mutant, collaborator-context effective reentry for start/stop/refresh, mandatory replay, namespace absence, and the fixed namespace known answer with separator/order mutations.
- Implementation: create one nonrecursive owner lock for snapshots/`desiredRunning`/sequence-and-append and one internal FIFO facade dispatcher. A collaborator-context effective call appends and returns so the active drain can continue. Add only internal test dependencies/the two-hook admission probe, the namespace, current-value publishers, and a factory backed by `NoOpLifecycle` plus the nil/idle/zero snapshot. Add no post-construction publication interface or drain contract; S1-05 owns both.
- Acceptance: start/stop transition once, repeated calls are filtered before the bridge, running refresh forwards once, stopped refresh no-ops, append cannot separate from reservation under the deterministic mutant, no collaborator runs under the lock, the namespace known answer is exact, and the factory cannot create any audited capability or escape through an unlisted helper.
...
- Suggested owner: ThorChainSwiftEngineer.
- Dependencies: step 3.
- Affected paths: `Tests/ThorChainKitTests/Fixtures/S1-01-tests.txt`, `Tests/ThorChainKitTests/Fixtures/S1-01-public-symbols.txt`, `Scripts/verify-s1-01.sh`, `Scripts/verify-s1-01-xunit.swift`, `.github/workflows/ci.yml`, and public documentation only as required by the approved gates.
- Test first: after all 18 methods exist, prove that an extra product, forbidden import, extra public symbol, missing/extra test, `XCTSkip`, wrong Xcode/Swift identity, strict-concurrency warning, iOS-16-only public API, public-consumer use of an internal symbol, each concrete forbidden factory capability, composition through an unlisted helper, and namespace separator/order mutations each fail their own named canary.
- Implementation: canonicalize/compare `dump-package`, generated symbol graph, `swift test list`, imports, and toolchain identity; run filtered XCTest with `--xunit-output` and parse exactly 18 executed entries with zero skip/disabled/failure/error; run the named Swift-5 strict-concurrency command; generate a `mktemp -d` Swift-tools-5.10 package with `.iOS(.v13)` and build it using only public `import ThorChainKit` with `IPHONEOS_DEPLOYMENT_TARGET=13.0 SWIFT_VERSION=5 SWIFT_STRICT_CONCURRENCY=complete SWIFT_TREAT_WARNINGS_AS_ERRORS=YES`.
- Acceptance: all audits are independently named and green; the discovered and executed allowlists are exactly 18 with no skipped/disabled case; host `swift build` is not an iOS compatibility substitute; no BigUInt-containing `Sendable` or `@unchecked Sendable` declaration exists. The committed Gimle report passes its operator-path rejection gate, and later-slice exact/subset/capability transitions remain named in S1-02…S1-05.
